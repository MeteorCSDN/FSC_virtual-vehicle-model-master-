...
% if ~isempty(getfullname(Simulink.findBlocksOfType(bdroot(block),'Reference Generator')))
if ~isempty(find_system(bdroot(block),'Name','Reference Generator'))    
    manType = get_param([bdroot(block) '/Driver/Lateral Ref + DRV/Reference Generator'],'manType');
    useDefValues = get_param([bdroot(block) '/Driver/Lateral Ref + DRV/Reference Generator'],'manOverride');
else
    manType = '';
    useDefValues = 'off';
end
...
if simStopped
    switch driverType
        ...
        case 'Open Loop - G29 Steering Wheel and Pedals'
            set_param([block '/Driver Commands'],'OverrideUsingVariant','3');
            SwitchInport(block,'Steer','Ground',[]);
            SwitchInport(block,'Accel','Ground',[]);
            SwitchInport(block,'Brake','Ground',[]);
            SwitchInport(block,'Gear','Ground',[]);    
        case 'Open Loop - F310 Gamepad'
            set_param([block '/Driver Commands'],'OverrideUsingVariant','4');
            SwitchInport(block,'Steer','Ground',[]);
            SwitchInport(block,'Accel','Ground',[]);
            SwitchInport(block,'Brake','Ground',[]);
            SwitchInport(block,'Gear','Ground',[]);            
